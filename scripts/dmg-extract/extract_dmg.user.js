...
(function () {
    ...
    function listener() {
        ...
        if (currentHash.includes('partydamage')|| currentHash.includes('#war/damage')) {

            if ($('#startButton').length == 0){
                $('#header_slide').append(
                    '<button id="startButton">Extract</button>'
                );
    ...
    function start() {
        ...
        var timer = setInterval(function () {
            try {
                ...
                const frontList = newFrontData[1]
                    ? 'partyFronts'
                    : 'soldierFronts';
